...
exports.addPayment = asyncErrorHandler(async (req, res, next) => {
  try {
    ...
    await Payment.create({ txnAmount: data.amount, orderId: orderId ,resultInfo: {resultStatus:"TXN_SUCCESS"} });
    //res.redirect(`https://localhost:3001/orders/${orderId}`);
    res.status(200).json({ msg: "Payment Successful",orderId:orderId });
